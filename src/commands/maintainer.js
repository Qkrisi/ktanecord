...
module.exports.run = async (client, message, args) => {
	...
	if(ArgList[0]=="add")
	{
		if(body["ScoreMaintainers"].includes(UserID)) return message.channel.send("The specified user is already a maintainer.")
		body["ScoreMaintainers"].push(UserID)
	}
	else
	{
		if(!body["ScoreMaintainers"].includes(UserID)) return message.channel.send("The specified user isn't a maintainer.")
